...
int points_to_voxel_3d_np2(py::array_t<DType> points, py::array_t<DType> voxels,
...
template <typename DType, int NDim>
int points_to_voxel_3d_np_indices_only(py::array_t<DType> points, py::array_t<DType> voxels,
                          py::array_t<DType> voxel_point_mask,
                          py::array_t<int> coors,
                          py::array_t<int> num_points_per_voxel,
                          py::array_t<int> coor_to_voxelidx,
                          std::vector<DType> voxel_size,
                          std::vector<DType> coors_range, int max_points,
                          int max_voxels, py::array_t<int> voxel_pt_indices_into_original_pt_cloud) {
  auto points_rw = points.template mutable_unchecked<2>();
  auto voxels_rw = voxels.template mutable_unchecked<3>();
  auto voxel_point_mask_rw = voxel_point_mask.template mutable_unchecked<2>();
  auto coors_rw = coors.mutable_unchecked<2>();
  ...
  auto N = points_rw.shape(0);
  auto num_features = points_rw.shape(1);
  auto voxel_pt_indices_into_original_pt_cloud_rw = voxel_pt_indices_into_original_pt_cloud.template mutable_unchecked<2>();
  // auto ndim = points_rw.shape(1) - 1;
  constexpr int ndim_minus_1 = NDim - 1;
  int voxel_num = 0;
  bool failed = false;
  ...
  for (int i = 0; i < N; ++i) {
    ...
    if (num < max_points) {
      ...
      // todo: for BEVSEG differentiable voxelization, only get the indices and perform the data moving in pytorch
      voxel_pt_indices_into_original_pt_cloud_rw(voxelidx, num) = i;
